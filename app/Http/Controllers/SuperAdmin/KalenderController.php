...
class KalenderController extends Controller
{
    ...
    public function export(Request $request)
    {
        ...
        $peminjaman = $query->orderBy('tanggal_mulai', 'asc')->get();
        
        $filename = 'kalender_' . $year . '_' . str_pad($month, 2, '0', STR_PAD_LEFT) . '_' . date('Y-m-d_H-i-s') . '.csv';
        ...
        $callback = function() use ($peminjaman) {
            ...
            foreach ($peminjaman as $p) {
                $barangList = $p->peminjamanBarangs->map(function($item) {
                    return $item->barang->nama_barang . ' (' . $item->jumlah_pinjam . ' unit)';
                })->implode('; ');
                
                $durasi = Carbon::parse($p->tanggal_mulai)->diffInDays(Carbon::parse($p->tanggal_selesai)) + 1;
